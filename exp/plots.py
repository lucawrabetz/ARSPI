...
class CustomPlotSession:
    ...
    def save_single_figure(
        ...
        max_x = group_data[self.x_axis].max()
        min_x = int(group_data[self.x_axis].min())
    ...
    def save_plots(self) -> None:
        ...
        for style in self.styles:
            ...
            for key, group in grouped:
                index += 1
                self.save_single_figure(pd.DataFrame(group), style, index, key)
...
def main():
    ...
    # COMMON CLEANUP
    common_cleanup(df)
    # Running time vs budget - BENDERS AND MIP, a curve per solver, a plot per k
    session1 = CustomPlotSession(
        ...
        ["policies"],
        [],
        [{"nodes": 79}, {"solver": "GREEDY"}],
        [{"policies": 0}, {"subsolver": "NONE"}, {"subsolver": "BENDERS"}],
    )
