...
pair<vector<vector<int> >, vector<Policy> > extendByOne(pair<vector<vector<int> >, vector<Policy> >& k_solution, AdaptiveInstance& m3, const LayerGraph& G) {
    ...
    for (int w=0; w<k; ++w){
        if (k_solution.second[w].objective < min_subset_obj) {
            min_subset_obj = k_solution.second[w].objective;
            min_subset_windex = w;
        }
    }
    ...
    cout << "windex: " << min_subset_windex << endl;
    cout << "subsets to keep: ";
    for (int q : k_solution.first[min_subset_windex]){cout << q << " ";}
    cout << endl;
    AdaptiveInstance m3_prime = AdaptiveInstance(&m3, k_solution.first[min_subset_windex]);
    m3_prime.set_policies(2);
    // cout << endl << endl;
    // cout << "m3 prime: " << endl;
    // m3_prime.printInstance(G);

    auto k_prime_solution = enumSolve(m3_prime, G);
