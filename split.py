...
FINAL_COLUMNS = ["set_name", "instance_name", "nodes", "arcs", "k_zero", "density", "scenarios", "budget", "policies", "solver", "unbounded", "optimal", "objective", "gap", "time", "cuts_rounds", "cuts_added", "avg_cbtime", "avg_sptime", "partition", "m_sym", "g_sym"]
...
def extract_mip(df):
    ...
    new_columns = {original_name: new_name for original_name, new_name in mapping.items() if new_name != ""}
...
def replace_instancenames(df):
    for key, val in SET_NAME_MAPPINGS.items():
        ...
        df.loc[mask, "instance_name"] = df.loc[mask, "instance_name"].str.replace(key, val)

def final_changes(df):
    df["optimal"] = df["optimal"].replace("NOT OPTIMAL", "NOT_OPTIMAL")
    new_df = df[df["policies"] != 0]
    replace_setnames(new_df)
    replace_instancenames(new_df)
    return new_df
...
def main():
    ...
    parser.add_argument('solver', metavar='S', type=str, nargs=1,
                        help='solver to extract')
    ...
    df.to_csv(results_path, header=True, index=False, columns=FINAL_COLUMNS)
